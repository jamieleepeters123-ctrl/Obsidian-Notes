...
- **Gap closed 6 Aug: `office-pi-config-backups/` had no off-box copy.** Same shape as the Portainer gap below — a dated snapshot pulled to this machine only isn't "backed up" until a copy exists somewhere else too. Pushed the `2026-08-06-office/` snapshot to the toolkit server as `~/backups/office-pi/bellcommander-office-pi-backup-20260806.tar.gz` — turns out that directory already existed with one earlier one-off pull (`...-20260730.tar.gz`, 30 Jul) that had never been mentioned here. Not yet automated (still a manual tar+scp after a session that changes live site data, same as the Pi convention itself) — worth scripting alongside `portainer-backup.sh` if this needs to stop depending on remembering to do it.
