...
- SSH access to the office Pi, the tablet, and the toolkit server — all key-based, no password known to Claude. Plus one new server-to-server key (6 Aug): the toolkit server's own `~/.ssh/office-pi-backup` (ed25519, no passphrase — has to run unattended from cron), authorized on the Pi for that key alone, scoped to what `office-pi-backup.sh` needs and nothing else.
